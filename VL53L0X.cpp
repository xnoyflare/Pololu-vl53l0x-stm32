...
HAL_StatusTypeDef VL53L0X::writeReg(uint8_t reg, uint8_t value)
{

	HAL_StatusTypeDef ret;
	uint8_t data[2] = {reg, value};
	// Send configuration register data
	ret = HAL_I2C_Master_Transmit(bus, (uint16_t)(address<<1), data, 2, HAL_MAX_DELAY);
	if(ret != HAL_OK)
		return ret;
...
HAL_StatusTypeDef VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
	HAL_StatusTypeDef ret;
	uint8_t data[3] = {reg, (value >> 8) & 0xFF, (uint8_t)value & 0xFF};

	// Send configuration register data
	ret = HAL_I2C_Master_Transmit(bus, (uint16_t)(address<<1), data, 3, HAL_MAX_DELAY);
	if(ret != HAL_OK)
		return ret;
...
HAL_StatusTypeDef VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
	HAL_StatusTypeDef ret;
	uint8_t data[5] = {reg, (value >> 24) & 0xFF, value >> 16, (value >> 8) & 0xFF, (uint8_t)value & 0xFF};

	// Send configuration register data
	ret = HAL_I2C_Master_Transmit(bus, (uint16_t)(address<<1), data, 5, HAL_MAX_DELAY);
	if(ret != HAL_OK)
		return ret;
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
	...
	// Send configuration register data
	ret = HAL_I2C_Master_Transmit(bus, (uint8_t)(address<<1), &reg, 1, HAL_MAX_DELAY);
	if(ret != HAL_OK)
		return 0;

	ret = HAL_I2C_Master_Receive(bus, (uint16_t)(address<<1)|0x01, &addata, 1, HAL_MAX_DELAY);
	if(ret != HAL_OK)
		return 0;
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
	...
	// Send configuration register data
	ret = HAL_I2C_Master_Transmit(bus, (uint16_t)(address<<1), &reg, 1, HAL_MAX_DELAY);
	if(ret != HAL_OK)
		return 0;

	ret = HAL_I2C_Master_Receive(bus, (uint16_t)(address<<1)|0x01, addata, 2, HAL_MAX_DELAY);
	if(ret != HAL_OK)
		return 0;

	return (addata[0] << 8) | addata[1];
}
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
	...
	ret = HAL_I2C_Master_Receive(bus, (uint16_t)(address<<1)|0x01, addata, 4, HAL_MAX_DELAY);
	if(ret != HAL_OK)
		return 0;

	return (addata[0] << 24) | (addata[1] << 16) | (addata[2] << 8) | addata[3];
}

// Write an arbitrary number of bytes from the given array to the sensor,
// starting at the given register
HAL_StatusTypeDef VL53L0X::writeMulti(uint8_t reg, uint8_t * src, uint8_t count)
{
	HAL_StatusTypeDef ret;
	uint8_t addata[count + 1];
	addata[0] = reg;
	...
	// Send configuration register data
	ret = HAL_I2C_Master_Transmit(bus, (uint16_t)(address<<1), addata, count + 1, HAL_MAX_DELAY);
	if(ret != HAL_OK)
		return ret;
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
	...
	ret = HAL_I2C_Master_Receive(bus, (uint16_t)(address<<1)|0x01, readdata, count, HAL_MAX_DELAY);
	if(ret != HAL_OK)
		return ;
...
uint16_t VL53L0X::readRangeContinuousMillimeters()
{
  ...
  {
    if ((io_timeout > 0 && ((uint16_t)(HAL_GetTick()) - timeout_start_ms) > io_timeout)) //checkTimeoutExpired()
    {
      did_timeout = true;
      return 65535;
    }
...
bool VL53L0X::getSpadInfo(uint8_t * count, bool * type_is_aperture)
{
  ...
  {
    if ((io_timeout > 0 && ((uint16_t)(HAL_GetTick()) - timeout_start_ms) > io_timeout)) { return false; }		//checkTimeoutExpired()
  }
  writeReg(0x83, 0x01);
  tmp = readReg(0x92);
